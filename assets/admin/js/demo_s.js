...
markersGroupKPI = new L.layerGroup();
            ...
            success:function(res) {
                ...
                if(res.status == 200) {
                    ...
                    if(call_connect >= 0 && call_connect <= 2) {
                        ...
                        $(".bg-facebook").css('background-color', 'yellow');
                    } else if(call_connect > 5) {
                        $(".bg-facebook").css('background-color', 'red');
                    }
                    
                    if(res.message['call_block_rate'] < 2) {
                        $(".bg-twitter").css('background-color', 'green');
                    } else if(res.message['call_block_rate'] >= 2 && res.message['call_block_rate'] <= 5) {
                        $(".bg-twitter").css('background-color', 'yellow');
                    } else if(res.message['call_block_rate'] > 5) {
                        $(".bg-twitter").css('background-color', 'red');
                    }
                    
                    if(res.message['call_drop_rate'] < 2) {
                        $(".bg-googleplus").css('background-color', 'green');
                    } else if(res.message['call_drop_rate'] >= 2 && res.message['call_drop_rate'] <= 5) {
                        $(".bg-googleplus").css('background-color', 'yellow');
                    } else if(res.message['call_drop_rate'] > 5) {
                        $(".bg-googleplus").css('background-color', 'red');
                    }
                    
                    if(res.message['mute_call_rate_downlink'] < 1) {
                        $(".bg-bitbucket").css('background-color', 'green');
                    } else if(res.message['mute_call_rate_downlink'] >= 1 && res.message['mute_call_rate_downlink'] <= 3) {
                        $(".bg-bitbucket").css('background-color', 'yellow');
                    } else if(res.message['mute_call_rate_downlink'] > 3) {
            ...
            success:function(res) {
                ...
                if(res.status == 200) {
                    ...
                    if(res.message['mute_locations_downlink']) {
                        loadMarkers(res.message['mute_locations_downlink'], '#04e199', ' Mute Uplink');
                    }
                    if(res.message['mute_locations_uplink']) {
                        loadMarkers(res.message['mute_locations_uplink'], '#da06f9', 'Mute Downlink');
                    }
                }
            }
        });
...
//            initAutocomplete();
        ...
        if(city != '') {
            $.ajax({
                type:'POST',
                url:'ajax/save_frm_data.php',
                data: {type: 'get_city_lat_lng', city: city},
                dataType:'json',
                beforeSend :function() {
//                    inProgress(btn);
                },
...
//                        toggleMarkersGroup();
        ...
        $.ajax({
            type:'POST',
            url:'ajax/save_frm_data.php',
            data: {type: 'get_network_details_mute', country: country, city: city, os: os, op: op, trend: trend, start_dt: start_dt, end_dt: end_dt},
            dataType:'json',
            beforeSend :function() {
//                inProgress(btn);
            },
...
//                    }
                    ...
                    if(res.message['mute_locations_downlink']) {
                        console.log(res.message['mute_locations_downlink'].length);
                        loadMarkersGroup(res.message['mute_locations_downlink'], '#04e199', 'mute_downlink');
                    }
                    toggleMarkersNew('mute_downlink', $("input:checkbox[name='mute_call'][value=mute_downlink]").prop("checked"));
                    
                    if(res.message['mute_locations_uplink']) {
                        console.log(res.message['mute_locations_uplink'].length);
                        loadMarkersGroup(res.message['mute_locations_uplink'], '#da06f9', 'mute_uplink');
                    }
                    toggleMarkersNew('mute_uplink', $("input:checkbox[name='mute_call'][value=mute_uplink]").prop("checked"));
                }
            }
        });
...
//                    toggleMarkersGroup();
                ...
                var lines = res.split('\n');
                var sinr_locations_total_count = lines.length;
                console.log('sinr-'+sinr_locations_total_count);
                var marker_point;
                var sinr_locations = [[], [], [], [], [], []];
                var sinr_locations_count = [[], [], [], [], [], []];
                var sinr_locations_percentage = [[], [], [], [], [], []];
                
                for(var line = 0; line < lines.length; line++) {                    
                    marker_point = lines[line].split(',');
                    
                    if(marker_point[2] < 0) {
                        sinr_locations[0].push(marker_point);
                    } else if(marker_point[2] >= 0 && marker_point[2] < 5) {
                        sinr_locations[1].push(marker_point);
                    } else if(marker_point[2] >= 5 && marker_point[2] < 10) {
                        sinr_locations[2].push(marker_point);
                    } else if(marker_point[2] >= 10 && marker_point[2] < 15) {
                        sinr_locations[3].push(marker_point);
                    } else if(marker_point[2] >= 15 && marker_point[2] < 20) {
                        sinr_locations[4].push(marker_point);
                    } else if(marker_point[2] >= 20) {
                        sinr_locations[5].push(marker_point);
                    }
                ...
                for(var i = 0; i < 6; i++) {
                    if(sinr_locations[i] == undefined) {
                        sinr_locations[i] = [];
                    }
                    sinr_locations_count[i] = sinr_locations[i].length;
                    sinr_locations_percentage[i] = parseFloat((sinr_locations_count[i]/sinr_locations_total_count) * 100).toFixed(1);
                }
                                
                var sinr_range_colors = ['#ee2020', '#f7a002', '#3f3fe3', '#f7f720', '#6fea6f', '#0cd00c'];
                
                $.each(sinr_locations, function( index, value ) {
                    loadMarkersGroup(value, sinr_range_colors[index], 'sinr');
                });
                
                for (var i = 0; i < legend_types.length; i++) {
                    if(legend_types[i] == 'sinr') {
                        if(legend_types[i] + '_locations_count') {
                            $.each(sinr_locations_count, function( index, value ) {
                                $("#" + legend_types[i] + "_count_" + (index+1)).html(' ( ' + value + ' ) ');
                            });
                        }

                        if(legend_types[i] + '_locations_percentage') {
                            $.each(sinr_locations_percentage, function( index, value ) {
                                $("#" + legend_types[i] + "_percentage_" + (index+1)).html(' ( ' + value + '%  ) ');
                            });
                        }
                    }
                ...
                toggleMarkersNew('sinr', $("input:checkbox[name='rf_list'][value=sinr]").prop("checked"));
            }
        });
    });
    ...
    $('#btn_export').on('click',function(){
        ...
        var form = $('<form name="export_form" method="post" action="ajax/save_frm_data.php"></form>');        
    ...
    if($('#btn_chart').length) {
//        $('#btn_chart').trigger("click");
    }
});

function inMapProgress(val,msg) {
    if(val){
        $('#plotingProgress').html(msg);
