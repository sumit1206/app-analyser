...
$sql = 
"SELECT Max(timestamp) as ts, Max(package_load_time) as plt from (".$baseSql.") as view1 WHERE package_load_time != '0' GROUP BY session order by Max(timestamp) ASC";
...
if($res){
	...
	if($numRows>0){
		...
		while ($row=pg_fetch_assoc($res)) {
			$firstBitTime = $row['plt'];
			$time = date('m/d/Y H:i:s', $row['ts']/1000);
			$details['package_load_time']=$firstBitTime;
            $details['date_time']=$time;	
            array_push($response["details"], $details);
            
			if($max_value== 0 && $min_value==0){
				$min_value = $firstBitTime;
				$max_value = $firstBitTime;
			}
			if($max_value < $firstBitTime){
				$max_value = $firstBitTime;
			}
			if($min_value > $firstBitTime){
				$min_value = $firstBitTime;
			}
            $sum = $sum + $firstBitTime;
            $count = $count + 1 ;						 		     
		}
		$avg = $sum / $count;
