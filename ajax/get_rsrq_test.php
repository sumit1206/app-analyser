...
//    $sql_rsrq = "select lat, lon lng, rsrq FROM voice_test.rf_data WHERE lat is not null and lat > 0 and lon is not null and lon > 0 and rsrq is not null and $op_rf $os_rf $ts_rf and $total_calls_bb_ids_cond";
    
    $sql_rsrq = "select * from (select lat, lon lng, rsrq, row_number() over (partition by round(lat*12000),round(lon*12000)) rowno from voice_test.rf_data where lat is not null and lat > 0 and lon is not null and lon > 0 and rsrq is not null and $op_rf $os_rf $ts_rf and $total_calls_bb_ids_cond) tbl where rowno = 1";
