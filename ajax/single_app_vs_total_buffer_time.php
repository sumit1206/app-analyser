...
if($res){
	...
	if($numRows>0){ 
		while ($row=pg_fetch_assoc($res)) {
          ...
          $sql2 = "SELECT SUM( buffer_time ) FROM  rf_data_with_buffer  WHERE  session  = '$session' ";
		  ...
		  $details['buffer_time']=$sum_of_buffer;
          $details['date_time']=$time;
		  array_push($response["details"], $details);
		  
        }
		$session_concat  = substr($session_concat, 0, -1);
		$sql4 = "SELECT MAX( buffer_time ) , MIN( buffer_time ) , AVG( buffer_time ) FROM  rf_data_with_buffer  WHERE  session  IN ($session_concat) AND  buffer_time  != '0'";
