...
//    $sql = "SELECT make, MODEL, OS, appVersion,IMEI, ts, LAT, LON, ACCURACY, TECH, SUB_TECH, ASU, rsrp, RSCP, RX_LEVEL, rsrq::double precision, ECIO, RX_QUAL, EARFCN,  UARFCN, ARFCN, sinr, MCC, MNC, LAC_TAC, cell_id, psc_pci, spn, DATA_STATE, SERVICE_STATE, RNC, CQI, FREQ, BAND, TA, call_state, CALL_DURATION, TEST_STATE, RSSI, SS, mute_of, mute_of, bb_id , call_drop, call_block FROM rf_details WHERE $op $os $ts";
    $sql = "SELECT * FROM rf_details WHERE $op $os $ts order by imei, ts";
...
//    $op = isset($_POST['op']) ? "lower(spn) = '".strtolower($_POST['op'])."'" : '';
    ...
    // Total Calls
    $sql_total_calls = "select call_type, count(distinct call_no) calls from rf_details where call_type is not null and $op $os $ts and $total_calls_bb_ids_cond group by call_type";
    ...
    // Mute Calls
    $sql_mute_calls = "select count(distinct call_no) mute_calls, min(mute_of) min_mute_sec, max(mute_of) max_mute_sec, avg(mute_of) avg_mute_sec from rf_details WHERE is_mute > 0 and call_state > 0 and $dwnlink_bb_ids_cond AND $op $os $ts UNION ALL select count(distinct call_no) mute_calls, min(mute_of) min_mute_sec, max(mute_of) max_mute_sec, avg(mute_of) avg_mute_sec from rf_details WHERE is_mute = 1 and call_state > 0 and $uplink_bb_ids_cond AND $op $os $ts";
...
//    $op = isset($_POST['op']) ? "lower(spn) = '".strtolower($_POST['op'])."'" : '';
    ...
    if($trend == 'day' || $trend == 'custom') {
        $no_of_days_or_hr = 31;
        $sql_call_connect_time_wise = "select extract(day from act_time + 0 * INTERVAL '1 second') as day, avg(setup_time)/1000 as avg_cst from rf_details  where setup_time > 0 and $op $os $ts and $total_calls_bb_ids_cond group by day";
    } elseif($trend == 'hour') {
        $no_of_days_or_hr = 24;
        $sql_call_connect_time_wise = "select extract(hour from act_time + 0 * INTERVAL '1 second') as hour, avg(setup_time)/1000 as avg_cst from rf_details  where setup_time > 0 and $op $os $ts and $total_calls_bb_ids_cond group by hour";
    }
...
//    $op = isset($_POST['op']) ? "lower(spn) = '".strtolower($_POST['op'])."'" : '';
    ...
    if($trend == 'day' || $trend == 'custom') {
        $no_of_days_or_hr = 31;
//        $sql_block_time_wise = "select extract(day from act_time) as day, count(distinct call_no) block_calls from rf_details where call_type = 1 and $op $os $ts and $total_calls_bb_ids_cond group by day";
        $sql_block_time_wise = "select count(DISTINCT case when (call_state > 0 and call_type = 1 and $dwnlink_bb_ids_cond) then call_no end) as block_calls, count(DISTINCT case when (call_state > 0 and $dwnlink_bb_ids_cond) then call_no end) as total_calls, extract(day from act_time + 0 * INTERVAL '1 second') as day from rf_details WHERE $op $os $ts group by day";
    } elseif($trend == 'hour') {
        $no_of_days_or_hr = 24;
//        $sql_block_time_wise = "select extract(hour from act_time) as hour, count(distinct call_no) block_calls from rf_details where call_type = 1 and $op $os $ts and $total_calls_bb_ids_cond group by hour";
        $sql_block_time_wise = "select count(DISTINCT case when (call_state > 0 and call_type = 1 and $dwnlink_bb_ids_cond) then call_no end) as block_calls, count(DISTINCT case when (call_state > 0 and $dwnlink_bb_ids_cond) then call_no end) as total_calls, extract(hour from act_time + 0 * INTERVAL '1 second') as hour from rf_details WHERE $op $os $ts group by hour";
    }
...
//    $op = isset($_POST['op']) ? "lower(spn) = '".strtolower($_POST['op'])."'" : '';
    ...
    if($trend == 'day' || $trend == 'custom') {
        $no_of_days_or_hr = 31;
//        $sql_drop_time_wise = "select extract(day from act_time) as day, count(distinct call_no) drop_calls from rf_details where call_type = 2 and $op $os $ts and $total_calls_bb_ids_cond group by day";
        $sql_drop_time_wise = "select count(DISTINCT case when (call_state > 0 and call_type = 1 and $dwnlink_bb_ids_cond) then call_no end) as block_calls, count(DISTINCT case when (call_state > 0 and call_type = 2 and $dwnlink_bb_ids_cond) then call_no end) as drop_calls, count(DISTINCT case when (call_state > 0 and $dwnlink_bb_ids_cond) then call_no end) as total_calls, extract(day from act_time + 0 * INTERVAL '1 second') as day from rf_details WHERE $op $os $ts group by day";
    } elseif($trend == 'hour') {
        $no_of_days_or_hr = 24;        
//        $sql_drop_time_wise = "select extract(hour from act_time) as hour, count(distinct call_no) drop_calls from rf_details where call_type = 2 and $op $os $ts and $total_calls_bb_ids_cond group by hour";
        $sql_drop_time_wise = "select count(DISTINCT case when (call_state > 0 and call_type = 1 and $dwnlink_bb_ids_cond) then call_no end) as block_calls, count(DISTINCT case when (call_state > 0 and call_type = 2 and $dwnlink_bb_ids_cond) then call_no end) as drop_calls, count(DISTINCT case when (call_state > 0 and $dwnlink_bb_ids_cond) then call_no end) as total_calls, extract(hour from act_time + 0 * INTERVAL '1 second') as hour from rf_details WHERE $op $os $ts group by hour";
    }
...
//    $op = isset($_POST['op']) ? "lower(spn) = '".strtolower($_POST['op'])."'" : '';
    ...
    if($trend == 'day' || $trend == 'custom') {
        $no_of_days_or_hr = 31;
        $sql_mute_calls_time_wise = "select count(DISTINCT case when (is_mute = 1 and call_state > 0 and $dwnlink_bb_ids_cond) then call_no end) as mute_calls, count(DISTINCT case when (call_state > 0 and $total_calls_bb_ids_cond) then call_no end) as total_calls, extract(day from act_time + 0 * INTERVAL '1 second') as day from rf_details WHERE $op $os $ts group by day";
    } elseif($trend == 'hour') {
        $no_of_days_or_hr = 24;
    ...
    if($trend == 'day' || $trend == 'custom') {
        $no_of_days_or_hr = 31;
        $sql_mute_calls_time_wise = "select count(DISTINCT case when (is_mute = 1 and call_state > 0 and $uplink_bb_ids_cond) then call_no end) as mute_calls, count(DISTINCT case when (call_state > 0 and $total_calls_bb_ids_cond) then call_no end) as total_calls, extract(day from act_time + 0 * INTERVAL '1 second') as day from rf_details  WHERE $op $os $ts group by day";
    } elseif($trend == 'hour') {
        $no_of_days_or_hr = 24;
        $sql_mute_calls_time_wise = "select count(DISTINCT case when (is_mute = 1 and call_state > 0 and $uplink_bb_ids_cond) then call_no end) as mute_calls, count(DISTINCT case when (call_state > 0 and $total_calls_bb_ids_cond) then call_no end) as total_calls, extract(hour from act_time + 0 * INTERVAL '1 second') as hour from rf_details  WHERE $op $os $ts group by hour";
    }
    //error_log($sql_mute_calls_time_wise."\n", 3, $log_file);
    $res_mute_calls_time_wise = pg_query($con, $sql_mute_calls_time_wise);
...
//    $op = isset($_POST['op']) ? "lower(spn) = '".strtolower($_POST['op'])."'" : '';
    ...
    $sql_mute_duration_histogram = "select min(total_mute_duration) min_mute_duration, max(total_mute_duration) max_mute_duration, avg(total_mute_duration) avg_mute_duration, sum(case when (total_mute_duration >= 0 and total_mute_duration < 2) then 1 else 0 end) AS range_1, sum(case when (total_mute_duration >= 2 and total_mute_duration < 4) then 1 else 0 end) AS range_2, sum(case when (total_mute_duration >= 4 and total_mute_duration < 6) then 1 else 0 end) AS range_3, sum(case when (total_mute_duration >= 6 and total_mute_duration < 8) then 1 else 0 end) AS range_4, sum(case when (total_mute_duration >= 8 and total_mute_duration < 10) then 1 else 0 end) AS range_5, sum(case when (total_mute_duration >= 10 and total_mute_duration < 12) then 1 else 0 end) AS range_6, sum(case when (total_mute_duration >= 12 and total_mute_duration < 14) then 1 else 0 end) AS range_7, sum(case when (total_mute_duration >= 14 and total_mute_duration < 16) then 1 else 0 end) AS range_8, sum(case when (total_mute_duration >= 16) then 1 else 0 end) AS range_9 from (select call_no, sum(mute_duration) as total_mute_duration from (select call_no, mute_no, max(mute_of) as mute_duration from rf_details where is_mute = 1 and call_state > 0 AND $uplink_bb_ids_cond AND $op $os $ts group by call_no, mute_no)tbl group by call_no)tbl2";
    //error_log($sql_mute_duration_histogram."\n", 3, $log_file);
    $res_mute_duration_histogram = pg_query($con, $sql_mute_duration_histogram);    
...
//    $op = isset($_POST['op']) ? "lower(spn) = '".strtolower($_POST['op'])."'" : '';
    ...
    $sql_cov_qua_mute = "select avg(rsrp::double precision) rsrp, avg(sinr::double precision) sinr, sum(mute_of)::float as mute_duration, mute_no from rf_details  where is_mute = 1 and mute_no > 0 and $dwnlink_bb_ids_cond and rsrp is not null and sinr is not null AND $op $os $ts group by mute_no";
...
//    $sql_cov_qua_mute = "select count(1) as mute_samples, min(mute_duration) min_mute_duration, max(mute_duration) max_mute_duration, avg(mute_duration) avg_mute_duration from (select call_no, rsrp, sinr, mute_of::float/1000 as mute_duration from rf_details where is_mute > 0 and call_state > 0 and $dwnlink_bb_ids_cond and rsrp is not null and rsrp != '' and rsrp != '-' and sinr is not null and sinr != '' and sinr != '-' AND $op $os $ts)tbl";
    $sql_cov_qua_mute = "select count(1) as mute_samples, min(mute_duration) min_mute_duration, max(mute_duration) max_mute_duration, avg(mute_duration) avg_mute_duration from (select mute_no, sum(mute_of)::float as mute_duration from rf_details  where is_mute = 1 and mute_no > 0 and $dwnlink_bb_ids_cond and rsrp is not null and sinr is not null AND $op $os $ts group by mute_no)tbl";
    //error_log($sql_cov_qua_mute."\n", 3, $log_file);
    $res_cov_qua_mute = pg_query($con, $sql_cov_qua_mute);    
    $mute_samples = $min_mute_duration_cov = $max_mute_duration_cov = $avg_mute_duration_cov = 0;
    if (pg_num_rows($res_cov_qua_mute) > 0) {
...
//    $op = isset($_POST['op']) ? "lower(spn) = '".strtolower($_POST['op'])."'" : '';
    ...
    $sql_rf_drop_block = "select distinct on (call_no) * from (select act_time as ts, rsrp, rsrq, sinr, psc_pci pci, cell_id, call_type, call_no from rf_details where call_type in (1,2) and call_state > 0 and $op $os $ts and $total_calls_bb_ids_cond)tbl order by call_no, ts desc";
...
//    $sql_cst = "select call_no, min(act_time) as cst from rf_details where call_no in (select call_no from rf_details where is_mute = 1 and mute_no > 0 and call_state > 0 and call_no > 0 and $op $os $ts and $total_calls_bb_ids_cond) and call_state > 0 and $op $os $ts and $total_calls_bb_ids_cond group by call_no";
//    $res_cst = pg_query($con, $sql_cst);
//    
//    $call_nos_cst = false;
//    if (pg_num_rows($res_cst) > 0) {
//        while ($row=pg_fetch_assoc($res_cst)) {
//            $call_nos_cst[$row['call_no']] = $row['cst'];
//        }
//    }
        
    $sql_rf_mute = "select call_no, mute_no, round(CAST(sum(mute_of) as numeric),1) as mute_duration, max(act_time+ 0 * INTERVAL '1 second') filter (where is_mute > 0 and call_state > 0) as ts, round(CAST(avg(rsrp) as numeric),1) rsrp, round(CAST(avg(sinr) as numeric),1) sinr, round(CAST(avg(rsrq) as numeric),1) rsrq, string_agg(DISTINCT psc_pci::varchar, ',') as pci, string_agg(DISTINCT cell_id::varchar, ',') as cell_id, round(CAST(max(mute_on_ms) as numeric)/1000,1) as mute_on from rf_details where is_mute = 1 and mute_no > 0 and call_state > 0 and $op $os $ts and $total_calls_bb_ids_cond group by call_no, mute_no order by ts";
...
//            if(isset($call_nos_cst[$row['call_no']])) {
//                $row['mute_on'] = strtotime($row['ts']) - strtotime($call_nos_cst[$row['call_no']]);
//            } else {
//                $row['mute_on'] = 0;
//            }
//            $row['mute_duration'] = number_format($row['mute_duration'], 1);
    ...
    if($call_nos) {
        $sql_get_voice_file_ids = "select call_no, file_name from (select call_no, sum(voice_files_id) voice_files_id from rf_details  where voice_files_id > 0 and $op $os $ts and $total_calls_bb_ids_cond group by call_no)tbl inner join voice_files_new on (voice_files_new.id = tbl.voice_files_id)";
...
//    $op = isset($_POST['op']) ? "lower(spn) = '".strtolower($_POST['op'])."'" : '';
    ...
    // Mute Calls
    $sql_mute_calls = "select count(distinct call_no) mute_calls from rf_details WHERE is_mute = 1 and call_state > 0 and $dwnlink_bb_ids_cond AND $op $os $ts UNION ALL select count(distinct call_no) mute_calls from rf_details WHERE is_mute = 1 and call_state > 0 and $uplink_bb_ids_cond AND $op $os $ts";
    //error_log($sql_mute_calls."\n", 3, $log_file);
    $res_mute_calls = pg_query($con, $sql_mute_calls);
