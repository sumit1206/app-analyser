...
$sql = 
"SELECT Max(timestamp) as ts, SUM(buffer_time) as bt from (".$baseSql.") as view1 WHERE package_load_time != '0' GROUP BY session order by Max(timestamp) ASC";
...
if($res){
	...
	if($numRows>0){
		...
		while ($row=pg_fetch_assoc($res)) {
			...
			$details['buffer_time']=$bufferTime;
            $details['date_time']=$time;	
            array_push($response["details"], $details);
            
			if($max_value== 0 && $min_value==0){
				$min_value = $bufferTime;
				$max_value = $bufferTime;
			}
			if($max_value < $bufferTime){
				$max_value = $bufferTime;
			}
			if($min_value > $bufferTime){
				$min_value = $bufferTime;
			}
            $sum = $sum + $bufferTime;
            $count = $count + 1 ;						 		     
