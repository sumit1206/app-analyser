...
//    $op = isset($_POST['op']) ? "lower(SPN) = '".strtolower($_POST['op'])."'" : '';
    ...
    $sql_sinr = "select lat, lon, sinr FROM rf_details WHERE lat is not null and lat > 0 and lon is not null and lon > 0 and sinr is not null and $op $os $ts and $total_calls_bb_ids_cond";
