...
$sql = 
"SELECT Max(timestamp) as ts, Max(no_of_buffer_per_session) as nbs from (".$baseSql.") as view1 WHERE package_load_time != '0' GROUP BY session order by Max(timestamp) ASC";
...
if($res){
	...
	if($numRows>0){
		...
		while ($row=pg_fetch_assoc($res)) {
			...
			$details['no_of_buffer_per_session']=$noOfBuffer;
            $details['date_time']=$time;	
            array_push($response["details"], $details);
            
			if($max_value== 0 && $min_value==0){
				$min_value = $noOfBuffer;
				$max_value = $noOfBuffer;
			}
			if($max_value < $noOfBuffer){
				$max_value = $noOfBuffer;
			}
			if($min_value > $noOfBuffer){
				$min_value = $noOfBuffer;
			}
            $sum = $sum + $noOfBuffer;
            $count = $count + 1 ;						 		     
