...
//    $op = isset($_POST['op']) ? "lower(operator) = '".strtolower($_POST['op'])."'" : '';
    ...
    if(!empty($city)) {
        ...
        if (pg_num_rows($city_lat_lng) > 0) {
            $rows = pg_fetch_assoc($city_lat_lng);
            $lat = $rows['latitude'];
            $lng = $rows['longitude'];
...
//    $sql_rsrp = "select lat, lon lng, rsrp FROM voice_test.rf_data WHERE lat is not null and lat > 0 and lon is not null and lon > 0 and rsrp is not null and $op_rf $os_rf $ts_rf and $total_calls_bb_ids_cond";
    
    $sql_rsrp = "select * from (select lat, lon lng, rsrp, row_number() over (partition by round(lat*12000),round(lon*12000)) rowno from voice_test.rf_data where lat is not null and lat > 0 and lon is not null and lon > 0 and rsrp is not null and $op_rf $os_rf $ts_rf and $total_calls_bb_ids_cond) tbl where rowno = 1";
    $res_rsrp = pg_query($con, $sql_rsrp);
    
    if (pg_num_rows($res_rsrp) > 0) {
        while ($row=pg_fetch_assoc($res_rsrp)) {
