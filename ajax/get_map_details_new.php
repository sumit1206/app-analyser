...
//    $op = isset($_POST['op']) ? "lower(SPN) = '".strtolower($_POST['op'])."'" : '';
    ...
    $sql_mute_locations = "select lat, lon, act_time ts, rsrp, sinr, cell_id from rf_details where is_mute = 1 and call_state > 0 and $dwnlink_bb_ids_cond and lat is not null and lat > 0 and lon is not null and lon > 0 and $op $os $ts";
    ...
    $sql_mute_locations = "select lat, lon, act_time ts, rsrp, sinr, cell_id from rf_details where is_mute = 1 and call_state > 0 and $uplink_bb_ids_cond and lat is not null and lat > 0 and lon is not null and lon > 0 and $op $os $ts";
    $res_mute_locations = pg_query($con, $sql_mute_locations);    
...
//    $sql_cdb_locations = "select act_time as ts, lat, lon, rsrp, rsrq, sinr, psc_pci pci, cell_id, call_type from rf_details where call_type in (1,2) and $op $os $ts";
    $sql_cdb_locations = "select distinct on (call_no) * from (select act_time as ts, lat, lon, rsrp, rsrq, sinr, psc_pci pci, cell_id, call_type, call_no from rf_details where call_type in (1,2) and call_state > 0 and $op $os $ts)tbl order by call_no, ts desc";
