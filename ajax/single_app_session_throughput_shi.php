...
$sql = 
"SELECT Max(timestamp) as ts, Max(session_throughput) as st from (".$baseSql.") as view1 WHERE package_load_time != '0' GROUP BY session order by Max(timestamp) ASC";
...
if($res){
	...
	if($numRows>0){
		...
		while ($row=pg_fetch_assoc($res)) {
			$sessionThroughput = $row['st'];
			$time = date('m/d/Y H:i:s', $row['ts']/1000);
			$details['session_throughput']=$sessionThroughput;
            $details['date_time']=$time;	
            array_push($response["details"], $details);
            
			if($max_value== 0 && $min_value==0){
				$min_value = $sessionThroughput;
				$max_value = $sessionThroughput;
			}
			if($max_value < $sessionThroughput){
				$max_value = $sessionThroughput;
			}
			if($min_value > $sessionThroughput){
				$min_value = $sessionThroughput;
			}
            $sum = $sum + $sessionThroughput;
            $count = $count + 1 ;						 		     
		}
		$avg = $sum / $count;
