...
if($res){
	...
	if($numRows>0){ 
		while ($row=pg_fetch_assoc($res)) {
		  ...
		  $sql1="SELECT  DISTINCT session ,  initial_buffr_time  FROM  rf_data_with_buffer  WHERE  running_app  = '$running_app' AND ( timestamp  BETWEEN '$unx_frm' AND '$unx_to') AND   initial_buffr_time  != '0'AND  wifi_state  !='1' AND  wifi_state !='-'  AND  spn  = '$operator' and state = '$city' and os = '$os'  ";
		  if($operator == 'WIFI'){
	             $sql1 = "SELECT  DISTINCT session ,  initial_buffr_time  FROM  rf_data_with_buffer  WHERE  running_app  = '$running_app' AND ( timestamp  BETWEEN '$unx_frm' AND '$unx_to') AND   initial_buffr_time  != '0' AND  wifi_state ='1' and state = '$city' and os = '$os'  ";
              }
